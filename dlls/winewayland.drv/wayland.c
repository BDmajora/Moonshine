                                   ...
                                   uint32_t version)
{
    ...
    else if (strcmp(interface, "yetios_cursor_manager_v1") == 0)
    {
        process_wayland.yetios_cursor_manager_v1 =
            wl_registry_bind(registry, id, &yetios_cursor_manager_v1_interface, 1);
    }
...
BOOL wayland_process_init(void)
{
    ...
    if (!process_wayland.yetios_cursor_manager_v1)
        WARN("Compositor doesn't support yetios_cursor_manager_v1 (boot cursor handoff won't work)\n");
