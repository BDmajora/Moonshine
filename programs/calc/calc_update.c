...
void ui_update_bit_display(HWND hwnd) {
    ...
    hb = GetDlgItem(hwnd, ID_DISP_BITS);
    if (!hb) return;
...
void ui_update_stat_display(HWND hwnd) {
    ...
    hl = GetDlgItem(hwnd, ID_DISP_HISTORY);
    if (!hl) return;
...
/* ── Implementation for Missing Linker References ── */
...
void ui_update_history_panel(HWND hwnd) {
    ...
    /* 
       Updated variable names based on compiler feedback:
       - history_count -> hist_count
       - history[i].str -> history[i] (it's a direct string pointer)
    */
    for (int i = 0; i < hist_count; i++) {
        SendMessageW(hl, LB_ADDSTRING, 0, (LPARAM)history[i]);
    }
    
    /* Scroll to bottom if items exist */
    if (hist_count > 0) {
...
void ui_update_prog_buttons(HWND hwnd) {
    ...
    BOOL enable_hex = (prog_base == BASE_HEX);
    BOOL enable_dec = (prog_base == BASE_DEC || enable_hex);
    BOOL enable_oct = (prog_base == BASE_OCT || enable_dec);
    
    /* 
       A-F: IDs are 110 through 115 in calc_defs.h.
       Even though C and E have _HEX suffixes, the numeric range remains sequential.
    */
    for (int id = ID_A; id <= ID_F; id++) {
        EnableWindow(GetDlgItem(hwnd, id), enable_hex);
    }
    
    /* 8-9: IDs 108 and 109 */
    EnableWindow(GetDlgItem(hwnd, ID_8), enable_dec);
    EnableWindow(GetDlgItem(hwnd, ID_9), enable_dec);
    
    /* 2-7: IDs 102 through 107 */
    for (int id = ID_2; id <= ID_7; id++) {
        EnableWindow(GetDlgItem(hwnd, id), enable_oct);
    }
}

/* ── End Implementation for Missing Linker References ── */
